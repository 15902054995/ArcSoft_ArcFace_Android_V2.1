...
public class DrawHelper {
    ...
    public static void drawFaceRect(Canvas canvas, DrawInfo drawInfo, int color, int faceRectThickness) {
        ...
        if (drawInfo.getName() == null) {
            ...
            String str = (drawInfo.getSex() == GenderInfo.MALE ? "男" : (drawInfo.getSex() == GenderInfo.FEMALE ? "女" : "不知"))
                    + ":"
                    + (drawInfo.getAge() == AgeInfo.UNKNOWN_AGE ? "不知" : drawInfo.getAge())
                    + "岁";
            String  alive = (drawInfo.getLiveness() == LivenessInfo.ALIVE ? "活的" : (drawInfo.getLiveness() == LivenessInfo.NOT_ALIVE ? "非活" : "不知"));
            canvas.drawText(str, rect.left, rect.top, paint);
            canvas.drawText(alive, rect.left , rect.bottom, paint);
        } else {
            paint.setStyle(Paint.Style.FILL_AND_STROKE);
            paint.setTextSize(rect.width() / 8);
            if (drawInfo.getName().length()>=4){
                canvas.drawText(drawInfo.getName(), rect.left, rect.top, paint);
            }
        }
    }
